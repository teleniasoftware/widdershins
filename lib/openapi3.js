    ...
    if (op.requestBody) {
        for (var rb in op.requestBody.content) {
            ...
            if (!data.bodyParameter.exampleValues.object) {
                ...
                if (op.requestBody.content[rb].examples && Object.keys(op.requestBody.content[rb].examples).length) {
                ...
                if (typeof data.bodyParameter.exampleValues.object === 'object') {
                    data.bodyParameter.exampleValues.json = common.safejson(data.bodyParameter.exampleValues.object, null, 2);
                }
                else {
...
function getParameters(data) {
    ...
    function stupidity(varname) {
        if (varname.endsWith('+')) varname = varname.split('+').join(''); // %2B
    ...
    for (let param of data.parameters) {
        ...
        if (pSchema) {
            ...
            if (typeof param.exampleValues.object === 'object') {
                param.exampleValues.json = common.safejson(param.exampleValues.object, null, 2);
            }
            else {
        ...
        for (let ess of effSecurity) {
            if (data.api.components.securitySchemes[ess]) {
                ...
                if (!existingAuth && ((secScheme.type === 'oauth2') || (secScheme.type === 'openIdConnect') ||
                    ...
                    authHeader.exampleValues.json = "'" + authHeader.exampleValues.object + "'";
                    data.allHeaders.push(authHeader);
                 } else if ((secScheme.type === 'apiKey') && (secScheme.in === 'cookie')) {
                        let authHeader = {};
                        ...
                        authHeader.isAuth = true;
                        authHeader.exampleValues = {};
                        authHeader.exampleValues.object = secScheme.name+'=API_KEY';
                        if (data.options.customApiKeyValue) {
                            authHeader.exampleValues.object = secScheme.name+'='+data.options.customApiKeyValue;
                        }
                        authHeader.exampleValues.json = "'" + authHeader.exampleValues.object + "'";
                        data.allHeaders.push(authHeader);
                    }
    ...
    // handle the case of malformed path templates which are missing as
    // in:path parameters. May be other cases to handle (amazonaws #323)
    uriTemplateStr = uriTemplateStr.split('+}').join('}');
    requiredUriTemplateStr = requiredUriTemplateStr.split('+}').join('}');
        ...
        if (!r.startsWith('x-')) {
            ...
            for (let ct in response.content) {
                ...
                if (contentType.schema) {
                    ...
                    if (contentType.schema.type === 'array' && contentType.schema.items) {
                        contentType.schema = contentType.schema.items;
                    }
                }
        ...
        if (!resp.startsWith('x-')) {
            ...
            for (let ct in response.content) {
                ...
                if (contentType.examples) {
                    ...
                    if (resp == "default" && !autoDone[autoCT]) {
...
function convertInner(api, options) {
    return new Promise(function (resolve, reject) {
        ...
        data.utils.safejson = common.safejson;
